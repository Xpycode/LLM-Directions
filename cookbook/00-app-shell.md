...
**Getting the key into the built plist is not trivial** — `INFOPLIST_KEY_*` under
`GENERATE_INFOPLIST_FILE = YES` silently drops non-allowlisted keys like this one, with
no build error. See [156-xcodegen-infoplist-allowlist.md](156-xcodegen-infoplist-allowlist.md)
for the allowlist gotcha, the `PlistBuddy` verify command, and three xcodegen/xcodeproj
fixes (an `info:` block, a checked-in `.xcodeproj`, and a build-setting merge).
