...
- **Keep the `-e` command string single-line.** On MaskTaster (2026-07-29, lftp via Homebrew) the multi-line `-e "` block exactly as written above failed *every* run with `mirror: Not connected` while `ls` on the same connection worked — it read as a transient network error and wasn't. Collapsing to one line (`-e "set sftp:auto-confirm yes; mirror -R --verbose --parallel=4 $STAGE_DIR $REMOTE_DIR; bye"`) fixed it permanently. If a mirror reports "Not connected" but `ls` connects, suspect the command string before the network.
- **Credentials: macOS Keychain, not hardcoded** — `security add-generic-password -s <site>-strato -a <user> -w` (interactive prompt; nothing in history), then in the script `SFTP_PASS="${STRATO_PASS:-$(security find-generic-password -s <site>-strato -w)}"`. Per-Mac (add once on each), never syncable, never committable. Older scripts (Bookmarks `deploy.sh`, LUCESUMBRARUM `ship.sh`) still hardcode passwords — migrate + rotate when touched.
