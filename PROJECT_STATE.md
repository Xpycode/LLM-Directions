...
- 2026-05-14: Swept other 7 candidate consumers (AutoRedact, MenuBarPLUS, MousePlus, SFTPmount, Group Alarms, KinoBerlin, apps.lucesumbrarum.com). Only AutoRedact had real contamination (**6 more files removed**); the other 6 had drift but no contamination — their "missing" entries are legit local suffix-variant sessions. Grand total: **29 contamination files cleaned across 4 consumers**. Side-fixed a whitespace bug in `sync-session-index.sh` (broke on paths like "Group Alarms"). 4 Syncthing sync-conflict files flagged for manual merge.
