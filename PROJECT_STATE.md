...
- 2026-05-14: Merged the 4 Syncthing sync-conflict files surfaced by the sweep. All 4 turned out to be the same pattern: base = strict superset (newer save that continued past where the conflict stopped); deleted the conflicts with zero information loss. MenuBarPLUS + AspectRatioUnifier now `✓ in sync`. 8 additional sync-conflicts surfaced OUTSIDE `docs/sessions/` (`.git/`, `.claude/`, source files, PROJECT_STATE) — left for separate per-file user decision; `.git/` files in particular should never be auto-handled.
