...
- 2026-05-14: Cleaned bootstrap contamination from 3 consumers (LUCESUMBRARUM, AvidMXFPeek, ePubReader) — 23 bit-identical-to-master session-log files removed via `cmp -s` + `rm` after dogfooding `sync-session-index.sh` on each. All 3 now `✓ in sync`. Revised understanding: contamination = "every master session at consumer's last sync time," not the audit's initial 6-date set. Same recipe applies to ~7 other consumers with smaller drift.
