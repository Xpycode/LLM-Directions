...
- **2026-07-02** — sat down on the M1 Max; `/arrive` flagged the alarming "14 behind *and* a dirty
...
- **Uncommitted (2026-07-02):** cookbook **#152** (`fs_usage` root-not-FDA fail-detection) + its one
  `PATTERNS-COOKBOOK.md` index row sit in the working tree, not yet committed. Pick up with
  `git add cookbook/152-*.md PATTERNS-COOKBOOK.md` → commit → `/depart` (stamp `Handoff-from: M1 Max`).
  Optionally add the missing **#151** index row in the same commit. Tree is otherwise on `origin/main`.
