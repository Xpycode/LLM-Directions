...
- [ ] **1.3 Prove bounded input and stop in a disposable target** → `C/Spikes/`, `V/stop-spike.md`
  - September 10 native continuation: [kernel capture and separate-process reload passed](verification/mac-control/shared-root-and-kernel-integration.md#fresh-acquisition--capture-and-reload-passed)
    once under the user's continuation and tool approval. Matching retained baseline/acquisition;
    marker unchanged, no launch authority. Preserve this successful transaction as well as all five
    failed ones and three observation journals. [Retained preflight CLI/caller](verification/mac-control/acquired-preflight-review.md)
    implemented and independently reviewed; persistent library staged. Arrange a same-Mac restart
    before one-shot inspection; no marker recreation/migration, initialization or foreground scope.
  ...
  - Current continuation: [kernel inventory contract/prototype](verification/mac-control/kernel-inventory-review.md)
    ...
    Shared-root selection is a prerequisite: directory-helper fallback can select different
    lock roots, reproduced with private real locks and mocked directory selection. Define and
    enforce the trusted existing runtime root before integration; preserve the unresolved marker.
    No live probe replacement or capture; task 1.3/Gate A remain open.
...
| 1 / 1.3 retained acquisition and restart preparation | 2026-09-10 | Capture/reload passed; task incomplete | Coordinator ran the authorized one-shot native acquisition, staged the persistent library and owned caller/tests/records/Git; fresh-context implementation agent owned retained inspection CLI/tests and separate reviewer approved integration. [478 broad passes/one skip; 52 focused passes; three wrapper tests; native capture/reload success](verification/mac-control/acquired-preflight-review.md). Marker unchanged; later-boot caller inspected but not run. Local branch checkpoint, no push. Next agreed same-Mac restart, then original-namespace inspection; Gate A open. |
